...
class VerifyCodeQuerySet(models.query.QuerySet):
    def _base_query(self, email=None, msisdn=None, token=None,
        ...
        if passcode:
            qs = qs.filter(passcode=passcode.upper())
        if challenge:
    ...
    def verified_unused(self, email=None, msisdn=None, token=None,
        ...
        qs = self._base_query(email, msisdn, token, passcode, challenge) \
...
class AbstractVerifyCode(models.Model):
    ...
    def validate(self):
        if self.is_used:
            raise ValidationError({'is_used': _("Has used on %s" % (self.update_at))})

        if self.is_verified:
            raise ValidationError({'is_verified': _("Has verified on %s" % (self.update_at))})

        if self.is_expired:
            raise ValidationError({'is_expired': _("Has expired on %s" % (self.update_at))})

        if timezone.now() >= self.valid_until:
            self.is_expired = True
            self.save(update_fields=['is_expired'])
            raise ValidationError({'valid_until': _("VerifyCode code expired on %s" % (self.valid_until))})
